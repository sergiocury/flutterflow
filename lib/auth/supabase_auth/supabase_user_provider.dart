...
class BrechoAppProSupabaseUser extends BaseAuthUser {
  BrechoAppProSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> brechoAppProSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = BrechoAppProSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
